...
/// resolves it or the wait elapses (defer).
fn wait_for_decision(app: &AppHandle, body: &str) -> PermDecision {
    ...
    let decision = rx.recv_timeout(wait).unwrap_or(PermDecision::Ask);
...
#[tauri::command]
pub fn resolve_permission(app: AppHandle, state: State<'_, AppState>, id: String, decision: String) {
    let dec = PermDecision::from_str(&decision);
    // Send first (unblocks the waiting hook), then drop the row.
...
# If Tablo is unreachable, prints {} so Claude Code falls back to its normal
# permission flow (never blocks the session).
PORT=__PORT__
