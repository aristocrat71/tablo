...
## Phase 4 — Permissions (approve/deny)

- Add Claude Code **hooks** (`PreToolUse`) to intercept tool calls.
- Establish a request/response loop: hook → widget (pending decision) → user taps approve/deny → decision returned to the blocked session.
- This is a networked request/response, not a file tail — will need a small local server or IPC channel the hook can hit.
